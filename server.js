...
// To prevent errors from Cross Origin Resource Sharing, we will set our headers to allow CORS with
// middleware like so:
...
router.route('/comments')
  ...
  .put(function(req, res) {
    Comment.findById(req.params.comment_id, function(err, comment) {
      ...
      (req.body.text) ? comment.text = req.bodytext : null;
      // save comment
      comment.save(function(err) {
        if (err)
        res.send(err);
  ...
  .delete(function(req, res) {
    //selects the comment by its ID, then remove it.
